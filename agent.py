...
def initialize_agent():
    ...
    llm_router = ChatOpenAI(model="gpt-5-mini", temperature=0)
...
def route_query(query, llm_router):
    ...
    raw_prompt = read_prompt_file("prompts/routing_prompt.txt")
    formatted_prompt = raw_prompt.format(query=query)
    response = llm_router.invoke(formatted_prompt)
...
def execute_rag(query, cluster_id, vectorstore, llm_responder):
    ...
    system_prompt_content = read_prompt_file("prompts/system_prompt.txt")
...
def run_agent_pipeline(query):
    ...
    print(f"The router (GPT-5-mini) assigned the query to cluster: {chosen_cluster}")
